...
function copyTestSkeletons(targetDir, framework) {
  const installed = [];

  if (framework === 'react-native' || framework === 'unknown') {
    // Android skeleton
    const androidDir = path.join(targetDir, 'android', 'app', 'src', 'androidTest', 'java', 'com', 'yourapp');
    ...
    if (!fs.existsSync(androidDest)) {
      fs.copyFileSync(path.join(TEMPLATES_DIR, 'FullAppUITest.kt'), androidDest);
  ...
  if (framework === 'flutter') {
    ...
    if (!fs.existsSync(flutterDest)) {
      fs.copyFileSync(path.join(TEMPLATES_DIR, 'app_test.dart'), flutterDest);
